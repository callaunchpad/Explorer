...
class PPO:
    def __init__(self, state_dim, action_dim, lr, epochs, batch_size, epsilon):
        ...
        self.pi_eval, self.vf_eval, _ = self._build_network(self.state_pl, 'pi', reuse=True)

        self.optimal_op = tf.squeeze(self.pi_eval.mode(), axis=0, name="optimal_action")
        self.sample_op = tf.squeeze(self.pi_eval.sample(1), axis=0, name="sample_action")
    ...
    def train(self, sess, s, a, r, adv):
        ...
        while True:
            try:
                _, _, loss, loss_pi, loss_val = sess.run(
                    [self.train_op, self.global_step, self.total_loss, self.loss_pi, self.loss_val])
                total_loss += loss
        ...
        print("====TIMES TRAINED: ", self.times_trained, "====")
    ...
    def get_action(self, sess, state, epsilon):
        ...
        #Epsilon greedy implementation below
        ...
        # else:
        #     # print(self.pi_eval)
        #     # print(self.optimal_op)
        #     action = sess.run(self.optimal_op, {self.state_pl: state[np.newaxis, :]})
        #     return action
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for episode in range(100000):
            ...
            while True:
                a = ppo.get_action(sess, obs, max(0.1, (50000-episode /100000)))
