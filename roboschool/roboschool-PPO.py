...
class PPO:
    def __init__(self, state_dim, action_dim, action_bound, lr, epochs, batch_size, epsilon):
        ...
        self.deterministic_op = pi_eval.mode()
    ...
    def get_action(self, sess, state, deterministic=False):
        if deterministic:
            return sess.run(self.deterministic_op, {self.state_pl: state[np.newaxis, :]})[0]
...
def main():
    ...
    with tf.Session() as sess:
        ...
        # CHANGE TO DIFFERENT ENV
        saver = tf.train.Saver()
        max_reward = 1961
        saver.restore(sess, "cheetah/model_6000_509.6928496536302.ckpt")

        #### COMMENT OUT THIS TO TRAIN ######
        for episode in range(1000000):
            ...
            while True:
                env.render()
                a = ppo.get_action(sess, obs, True)
        ...
        for episode in range(5100, 1000000):
            obs = env.reset()
            total_reward, num_steps = 0, 0
            terminal = False
            ...
            if episode % 100 == 0 and max_reward < total_reward:
                saver.save(sess, "./ant/model_" + str(episode) + "_" + str(total_reward) + ".ckpt")
